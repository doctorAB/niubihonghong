...
$(function() {//主函数，触发事件
    var svg = d3.select("svg");//找画板
    // 绑定拖拽
    $('#left-wrapper .node').draggable({
        helper: "clone",//jq的属性配置
        addClass: false,
        ...
        stop: function (e, ui) {
            ...
            g.selectAll("circle.output").call(//对图形的输出连线进行监控
                d3.drag()
            ...
            g.selectAll("circle.input")//对图形的输入连线进行监控
...
var translate = null;
var drawLine = false;
function linestarted() {//绘制连线
    drawLine = false;
    // 当前选中的circle
    var anchor = d3.select(this);
    // 当前选中的节点
    var node = d3.select(this.parentNode);//获得所需属性
...
function lineended(d) {//线条绘制结束，写入一些属性，初始化
    drawLine = false;
...
var dy = 0;
var dragElem = null;
function dragstarted() {//移动前获得一些东西
...
function dragended() {//结束移动，初始化
    dx = dy = 0;
    dragElem = null;
}

function addNode(svg, node) {//修改svg，通过增添标签来，绘制的具体过程
    var g = svg.append("g")
               .attr("class", "node")
